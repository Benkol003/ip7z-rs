...
fn check_codecs() -> Result<(), Box<dyn Error>> {
    unsafe {
        ...
        for i in 0..num {
            ...
            let id = Codecs::try_from(id)
                .map_err(|e| format!("Unknown codec: {}", e))
                .unwrap();
        ...
        let missing_codecs: Vec<_> = Codecs::iter()
            .filter(|x| !supported_codecs.contains(x))
            .collect();
